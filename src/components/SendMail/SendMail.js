...
function SendMail() {
    ...
    const sendEmail = async(msg) =>{
        ...
        const resp = await axios.post('https://QuMail-ml.herokuapp.com/predict', {message: cleanMsg},config)
    ...
    const generateKeywords = async (formData) => {
        ...
        const resp = await axios.post('https://QuMail-ml.herokuapp.com/keywords', {message: cleanMsg,n:5},config)
    ...
    const checkIfEmailExists = async (email) => {
        const snapshot = await db.collection('users').where('QuMails','==',email).limit(1).get()
    ...
    const onSubmit = async (formData) => {
        ...
        if(emailExists){   
            db.collection('QuMails').add({
                to: formData.to,
                from: auth.currentUser.email,
                subject:  encrypt(formData.subject, generateRoomName(auth.currentUser.email)),
                message: encrypt(addData, generateRoomName(auth.currentUser.email)),
            ...
            toast.error("Cannot send QuMail to "+formData.to)
